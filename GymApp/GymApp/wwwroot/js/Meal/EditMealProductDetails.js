import { DeleteEditProduct, UpdateEditMealSummary, EditEditIcon } from './UpdateMealSummaryAndDeleteProduct.js';
...
$(document).on('click', '#searchResults a', function (e) {
    ...
    $.ajax({
        url: '/Meal/AddProductToEditViewModel?productId=' + productId,
        ...
        success: function (productAttributes) {
            ...
            $iconLink.on('click', function () {
                DeleteEditProduct(jsonData, rowId).then(function () {
                    UpdateEditMealSummary();
                })
            });
            EditEditIcon($EditIconLink);
